...
export default {
	...
	computed: {
		mapLoading() {
			return this.photosLoading || this.contactsLoading || this.favoritesLoading || this.tracksLoading || this.myMapsLoading
		},
		// slider
		minDataTimestamp() {
			return Math.min(
	...
	methods: {
		...
		onContactAddressDelete(contact, save = true) {
			network.deleteContactAddress(contact.BOOKID, contact.URI, contact.UID, contact.ADR, this.myMapId).then((response) => {
				if (save) {
					this.saveAction({
						type: 'contactDelete',
						content: contact,
		...
		onContactPlace(e, save = true) {
			network.placeContact(e.contact.BOOKID, e.contact.URI,
				...
				e.address, e.addressType, this.myMapId
			).then((response) => {
				if (save) {
					this.saveAction({
						type: 'contactPlace',
